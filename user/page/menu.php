...
if (!isset($_SESSION["login_user"])) {
    ...
    $query_tampil = "SELECT id_menu, nama_menu, harga_menu, deskripsi, foto_menu_path FROM menu WHERE UPPER(kategori_menu) = '$kategori_terpilih'";
        ...
        <?php while ($row = mysqli_fetch_assoc($result_tampil)) : ?>
            <div class="menu-item">
                ...
                <div class="button-container">
                    <button type="button" onclick="addToKeranjang(<?php echo $row['id_menu']; ?>, '<?php echo $row['nama_menu']; ?>', <?php echo $row['harga_menu']; ?>)">
...
<script>
    function addToKeranjang(id_menu, nama_menu, harga_menu) {
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "../fitur/keranjang.php?tambah_ke_keranjang&id_menu=" + id_menu + '&nama_menu=' + encodeURIComponent(nama_menu) + '&harga_menu=' + harga_menu, true);
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                // Tambahkan notifikasi di sini
                showNotification('Menu "' + nama_menu + '" telah ditambahkan ke keranjang');
    ...
    function showNotification(message) {
        // Tampilkan notifikasi, misalnya dengan alert atau menggunakan library notifikasi
        alert(message);
    }
